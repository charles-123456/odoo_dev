    ...
    def create_invoice(self):
        ...
        for obj in inv_obj:
            var = [
                ...
                'Date:'+str(obj.date),
                'Employee:'+str(obj.project_id.timesheet_ids.mapped('employee_id.name')[0])]
            if self.is_task :
                var[0] = "Employee:" + str(obj.project_id.timesheet_ids.mapped('employee_id.name')[0])
            else :
                var[0] = ""
            if self.is_project :
            ...
            duplicate = obj.search([('timesheet_invoice_id','=',invoice_exist_check)])
